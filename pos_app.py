...
class POSApp(tk.Tk):
    def __init__(self) -> None:
        ...
        self.current_sale: List[Tuple[Product, int]] = []
        self.session_sales: List[List[Tuple[Product, int]]] = []
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Button(top_frame, text="Abrir caja", command=self.open_cash).grid(row=0, column=0, padx=5)
        ttk.Button(top_frame, text="Cerrar caja", command=self.close_cash).grid(row=0, column=1, padx=5)
        ttk.Button(top_frame, text="Configurar impresora", command=self.configure_printer).grid(row=0, column=2, padx=5)

        ttk.Button(top_frame, text="Cargar productos CSV", command=self.load_products_csv).grid(row=0, column=3, padx=5)
        ttk.Label(top_frame, text="Caja: ", font=("TkDefaultFont", 10, "bold")).grid(row=0, column=4, sticky="e")
        ...
        self.cash_status_label.grid(row=0, column=5, sticky="w")
        ...
        sale_frame.rowconfigure(1, weight=1)
        ...
        ttk.Button(qty_frame, text="Añadir producto", command=self.add_product_to_sale).grid(row=0, column=2, padx=5)
        ttk.Button(qty_frame, text="Cobrar venta", command=self.finish_sale).grid(row=0, column=3, padx=5)

        self.sale_tree = ttk.Treeview(sale_frame, columns=("ref", "desc", "qty", "price", "total"), show="headings")
        for col, title, width in [
            ("ref", "Referencia", 120),
        ...
        self.sale_tree.grid(row=1, column=0, sticky="nsew")
        ...
        sale_scroll.grid(row=1, column=1, sticky="ns")

        self.sale_total_var = tk.StringVar(value="Total: $0.00")
        ttk.Label(sale_frame, textvariable=self.sale_total_var, font=("TkDefaultFont", 11, "bold")).grid(row=2, column=0, sticky="e", padx=5, pady=5)
    ...
    def add_product_to_sale(self) -> None:
        ...
        self.current_sale.append((product, qty))
    ...
    def _refresh_sale_tree(self) -> None:
        ...
        total = 0.0
        for product, qty in self.current_sale:
        ...
        self.sale_total_var.set(f"Total: ${total:,.2f}")
    ...
    def finish_sale(self) -> None:
        ...
        if not self.current_sale:
            ...
            return
        sale_copy = list(self.current_sale)
        self.session_sales.append(sale_copy)
        self._print_ticket_for_sale(sale_copy)
        self.current_sale = []
        self._refresh_sale_tree()
    ...
    def _build_cash_summary(self) -> str:
        ...
        for sale in self.session_sales:
            for product, qty in sale:
                aggregated[product.reference] = aggregated.get(product.reference, 0) + qty
    ...
    def _print_ticket_for_sale(self, sale: List[Tuple[Product, int]]) -> None:
        lines = ["*** Venta ***", datetime.now().strftime("%d/%m/%Y %H:%M"), ""]
        total = 0.0
        for product, qty in sale:
            subtotal = product.price * qty
            total += subtotal
            lines.append(f"{product.reference} x{qty} - ${subtotal:,.2f}")
        lines.append("")
    ...
    def _print_text(self, text: str) -> None:
        ...
        ticket_file = os.path.join(TICKET_DIR, f"ticket_{timestamp}.txt")
